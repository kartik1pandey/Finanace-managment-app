...
import { saveRawMCPData } from "@/lib/dataStore";
  ...
  const checkLoginStatus = async (sessionId: string) => {
    ...
    try {
      ...
      if (!data.login_required && data.result) {
        // Save the entire JSON result to Supabase
        const userEmail = typeof window !== "undefined" ? localStorage.getItem("userEmail") : null;
        if (userEmail) {
          try {
            await saveRawMCPData(userEmail, data.result, sessionId);
          } catch (saveErr) {
            console.error("Failed to save data to Supabase:", saveErr);
          }
        }
